...
class involution(nn.Module):
    def __init__(self, channels, ksize, stride, group_ch, red_ratio=4):
        super(involution, self).__init__()
        
        self.channels = channels
        self.ksize = ksize
        self.stride = stride
        self.groups = channels//group_ch
        self.ch = channels//red_ratio//8
        
        self.conv1 = nn.Conv2d(channels, self.ch*6, kernel_size=1, stride=1, padding=0)
        self.conv2 = nn.Conv2d(channels, self.ch, kernel_size=3, stride=1, padding=1)   
        self.conv3 = nn.Conv2d(channels, self.ch, kernel_size=3, stride=1, padding=2, dilation=2)                   
        self.act   = nn.Sequential(
                     nn.BatchNorm2d(channels//red_ratio),
                     nn.ReLU(inplace=True))
        self.fuse  = nn.Conv2d(channels//red_ratio, ksize*ksize*self.groups, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x):
        ...
        f2 = self.conv2(x)
        f3 = self.conv3(x)
        feature = self.act(torch.cat([f1, f2, f3], 1))
        weight = self.fuse(feature)
    
        b, c, h, w = weight.shape
        weight = weight.view(b, self.groups, self.ksize, self.ksize, h, w)
        out = _involution_cuda(x, weight, stride=self.stride, padding=(self.ksize-1)//2)
        return feature, out
